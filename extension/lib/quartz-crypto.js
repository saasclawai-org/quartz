// Adapted from wallet/crypto.js — same derivation (BIP39 → PBKDF2 2048 → SLIP-10 m/44'/789'/0'/0'/0' → Ed25519).
// Runs in BOTH window contexts (wallet page) and the MV3 service worker (no window there).
...
async function loadCrypto() {
    if (!nacl) {
        nacl = (typeof window !== 'undefined' && window.nacl) || self.nacl;
        if (!nacl) throw new Error('tweetnacl failed to load');
    }
    if (!wordlist) {
        wordlist = WORDLIST;
        if (!wordlist) throw new Error('wordlist missing');
    }
}
